...
jQuery(document).ready(function($) {
	...
	$(".install").on('click', function (e) {
    	...
    	var ajaxurl = eightmediWelcomeObject.ajaxurl;
    	...
    	var plNonce = newPlhref[newPlhref.length-1];
    	if(plNonce==''){
    		var plNonce = eightmediWelcomeObject.admin_nonce;
    	}
		...
		$.ajax({
			...
			url: ajaxurl,
			data: {
				action: 'eightmedi_lite_plugin_installer',
				plugin: plugin,
			...
			success: function(response) {
		   		...
		   		if(response == 'success'){
				   		...
				   		el.html(eightmediWelcomeObject.installed_btn);
	...
	$('.install-offline').on('click', function (e) {
		...
		$.ajax({
			...
			url: ajaxurl,
			data: {
				action: 'eightmedi_lite_plugin_offline_installer',
	...
	$(".activate").on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'eightmedi_lite_plugin_activation',
	   			plugin: plugin,
	   			nonce: eightmediWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
	   		success: function(response) {
		   		if(response){
	...
	$('.activate-offline').on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'eightmedi_lite_plugin_offline_activation',
	   			plugin: plugin,
	   			nonce: eightmediWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
