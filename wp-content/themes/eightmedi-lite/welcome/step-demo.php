...
<div class="warning-msg"><?php echo __('Please install Recommended Plugins for demo to be imported completely.<br /> And Make sure you download demo in a fresh install. <br /> Your Old Data might be deleted.','eightmedi-lite');?></div>
<?php
...
foreach($eightmedi_lite_req_plugins as $slug=>$plugin) :
	if($plugin['bundled'] == false) {
		...
		<div class="action-tab warning">
			<h3><?php printf( esc_html__("Install : %s Plugin", 'eightmedi-lite'), $plugin['name'] ); ?></h3>
			<p><?php echo __('Please check the plugins folder inside theme and upload the zip of plugins from plugin uploader.','eightmedi-lite');?></p>
		</div>
		<?php
	} else {
